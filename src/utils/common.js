...
export function calMoneyQuantile(number, hasPoint = true) {
  ...
  const pointNum = arr[1].indexOf('.') < 0 ? '' : arr[1].substr(arr[1].indexOf('.') + 1, arr[1].length);
  let num = pointNum === '' ? arr[1] : arr[1].substr(0, arr[1].length - pointNum.length - 1);
  let result = '';
  while (num.length > 3) {
...
export function toFixed(number, length, hasPer = false) {
  ...
  const num = dot > -1 && str.substr(dot + length + 1, 1) >= 5 ? Math.round(numberPlus * multiple) + 1 : Math.round(numberPlus * multiple);
